...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        # 		mask = np.exp(-distance/(2*sigma2))
        # 		sumofmask+ = mask
        # for y in range(5):
        # 	for x in range(5):
        # 		mask = mask/sumofmask
        mask = cv2.GaussianBlur(srcImage,(5,5),0.5)
        ...
        lamda=((Sx2+Sy2)+sqrt(4*Sxy*Sxy+(Sx2-Sy2)*(Sx2-Sy2)))/2;
		orientationImage = np.arctan((lamda-Sx2)/Sxy) + np.pi;
    ...
    def computeLocalMaxima(self, harrisImage):
        ...
        destImage = np.allclose(ismax, harrisImage)
